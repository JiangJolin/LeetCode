...
class Twitter {
    ...
    class Post{
        public int user_id, post_id;
        public Post(int user, int post){
            user_id = user;
            post_id = post;
        }
    }
    ...
    public void postTweet(int userId, int tweetId) {
        ...
        Post new_post = new Post(userId, tweetId);
        posts.addFirst(new_post);
    }
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        User x = users.get(userId);
        int count = 0;
        Iterator it = posts.iterator();
        Post temp;
        while(count < 10 && it.hasNext()){
            temp = (Post)it.next();
            if(temp.user_id == x.user_id || x.follows.contains(temp.user_id)){
                count ++;
                feed.add(temp.post_id);
            }
        }
        return feed;
    }
    ...
    public void follow(int followerId, int followeeId) {
        ...
        User x = users.get(followerId);
        x.follows.add(followeeId);
    }
    ...
    public void unfollow(int followerId, int followeeId) {
        ...
        if(!users.containsKey(followeeId)) users.put(followeeId, new User(followeeId));
        User x = users.get(followerId);
        x.follows.remove((Integer)followeeId);
    }
}
